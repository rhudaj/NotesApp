...
export function Section(props: {
	...
	onNewSection: (secKey: string, replace: boolean, cname?: string) => void,
	...
	const onKeyDownCapture = (e: React.KeyboardEvent<HTMLDivElement>) => {
		...
		if (e.key === "Enter") {
			...
			if (commandMatch) {
				const cname = commandMatch[0].slice(1);	// remove the leading "/"
				if ( CUSTOM_COMPONENTS[cname] ) {
					...
					props.onNewSection(props.secKey, doReplace, cname);
		...
		// ####################### BULLET LIST #######################
			...
			if (TXT === "-") {
				...
				if (newTxt === "") {
					...
					props.onNewSection(props.secKey, true, "list");
